...
- **Discord** is now the official upstream client, packaged directly by cairn (`pkgs/discord/`). Vesktop is gone. The new derivation tracks Discord's modular distro format — host bundle plus 12 separately-versioned `.node` modules — with hashes regenerated by `pkgs/discord/update.sh` against `updates.discord.com/distributions/app/manifests/latest`. Stable channel, currently pinned to 1.0.140 (ahead of nixpkgs's 1.0.137). The launcher is wrapped to enable native Wayland (`--ozone-platform=wayland`, IME, decorations) when `NIXOS_OZONE_WL` is set, symlink-stage modules into `~/.config/discord/$VERSION/modules` on first run, and flip `SKIP_HOST_UPDATE` so Discord doesn't try to overwrite the nix store with a deb. `Mod+D` rebinds from `vesktop` to `discord`. The motivation is ToS: vesktop's bundled client modifications violate Discord's terms; the cairn-packaged client is vanilla.

  **Trade-offs vs. vesktop**: Krisp premium noise cancellation fails its signature check because `autoPatchelfHook` rewrites the dynamic section of the prebuilt `.node` module — Discord's stock noise suppression still works. No Vencord plugins. Screen sharing relies on Discord's native pipewire path rather than vesktop's custom integration.

### Removed

- **`vesktop`** from `desktop.streaming.enable`. Listed Vesktop references scrubbed from `home/desktop/niri-keybinds.nix`, `modules/desktop/default.nix`, `docs/APPLICATIONS.md`, and `docs/THEMING.md`.
