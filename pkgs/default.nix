...
in
{
  ...
  flake.overlays.default =
    ...
    // {
      ...
      # NOTE: cairn used to rebuild brave/brave-origin here to restore `.override`
      # on the flavors, because the old make-brave.nix applied the release args
      # non-overridably. Upstream flipped make-brave.nix's two-stage order so the
      # deps stage (which carries commandLineArgs) is the callPackage'd one — the
      # flavors are overridable again on their own. The workaround is gone; if
      # `programs.brave.commandLineArgs` ever breaks again, that's why.
